...
Parser::Expression Parser::emit_unary_op(int op, Parser::Expression expr) {
    ...
    free_expr_reg(expr);
    std::uint8_t reg = current_scope_->first_free_reg_++;
    auto opcode = static_cast<Instruction::Opcode>(
            op - Parser::NEG + Instruction::NEG);
...
Parser::Expression Parser::emit_commutative_op(int op, Parser::Expression lhs,
    ...
    if (use_imm_instruction) {
        ...
        free_expr_reg(lhs);
        opcode = static_cast<Instruction::Opcode>(
                op - Parser::ADD + Instruction::ADDRI);
        c = static_cast<std::int8_t>(rhs.value());
    } else {
        ...
        free_expr_reg(rhs);
        opcode = static_cast<Instruction::Opcode>(
                op - Parser::ADD + Instruction::ADDRR);
        c = rhs.reg();
    }
    std::uint8_t reg = current_scope_->first_free_reg_++;
...
Parser::Expression Parser::emit_noncommutative_op(int op,
        Parser::Expression lhs, Parser::Expression rhs) {
    ...
    if (!rhs.has_reg() &&
        ...
        free_expr_reg(lhs);
        opcode = static_cast<Instruction::Opcode>(
                op - Parser::SUB + Instruction::SUBRI);
        b = lhs.reg();
        c = static_cast<std::int8_t>(rhs.value());
    } else if (!lhs.has_reg() &&
            lhs.value() >= std::numeric_limits<std::int8_t>::min() &&
        ...
        free_expr_reg(rhs);
        opcode = static_cast<Instruction::Opcode>(
                op - Parser::SUB + Instruction::SUBIR);
        b = static_cast<std::int8_t>(lhs.value());
        c = rhs.reg();
    } else {
        rhs = expr_to_any_reg(rhs);
        lhs = expr_to_any_reg(lhs);
        ...
        free_expr_reg(rhs);
        opcode = static_cast<Instruction::Opcode>(
                op - Parser::SUB + Instruction::SUBRR);
        b = lhs.reg();
        c = rhs.reg();
    }
    std::uint8_t reg = current_scope_->first_free_reg_++;
...
Parser::Expression Parser::emit_call(std::size_t symbol_id, std::uint8_t reg,
    ...
    bytecode_.push_back(Instruction::make_abc(Instruction::CALL, reg, num_args,
                0));
    return expr;
}
...
void Parser::emit_return(Parser::Expression expr) {
    if (!expr.has_reg() &&
            ...
            expr.value() <= std::numeric_limits<std::int16_t>::max()) {
        bytecode_.push_back(Instruction::make_ad(Instruction::RETI, 0,
                    expr.value()));
    } else {
        expr = expr_to_any_reg(expr);
        free_expr_reg(expr);
        bytecode_.push_back(Instruction::make_abc(Instruction::RETR, expr.reg(),
                    0, 0));
    }
}
...
std::size_t Parser::parse_parameters() {
    ...
    for (;;) {
        ...
        if (UNLIKELY(lexer_.token() != ',')) {
            std::fprintf(stderr,
                    "Error in line %zu: expected ')' or ',' but got'",
...
void Parser::parse_block() {
    ...
    // Parse statements.
    while (lexer_.token() != '}' && parse_statement() == 0);
    lexer_.check_and_consume_token('}');
    // End the scope.
...
void Parser::parse_assignment_or_call() {
    // Is it an assignment or a function call?
    std::size_t symbol_id = lexer_.token_attribute().sz;
    lexer_.check_and_consume_token(Lexer::IDENTIFIER);
    std::size_t reg = find_variable_reg(symbol_id);
    if (reg != std::numeric_limits<std::size_t>::max()) {
...
void Parser::parse_fn() {
    ...
    // FIXME: GCC 6.1.1 bug in complete_ctor_at_level_p
    // auto expr = Parser::Expression::make_reg(0);
...
void Parser::parse() {
    // Emit the program prolog.
    std::experimental::string_view main("main", sizeof("main") - 1);
